...
import { getLocale, localizedToolPath, LOCALES, type LocaleCode } from '@/data/internationalSeo';
...
function getToolIcon(cat: string) {
  switch (cat) {
...
export default async function LocalizedToolsIndex({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <LocalizedToolsClient locale={locale.code} ui={ui} />;
}

function LocalizedToolsClient({ locale, ui }: { locale: LocaleCode; ui: ReturnType<typeof getLocalizedUi> }) {
  ...
  const filteredTools = useMemo(() => TOOLS_REGISTRY.filter((tool) => {
    ...
    const localizedName = getLocalizedToolName(tool, locale).toLowerCase();
    const matchesSearch =
        ...
        <section className="mx-auto max-w-7xl px-4 sm:px-6">
            ...
            {CATEGORIES.map((category) => {
              ...
              return (
                <button key={category} onClick={() => setSelectedCategory(category)}
                  className={`whitespace-nowrap rounded-xl px-4 py-2 text-xs font-semibold transition-all ${isActive ? 'bg-violet-600 text-white shadow-md shadow-violet-500/20' : 'bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-800 hover:border-violet-300 dark:hover:border-zinc-700'}`}>
                  {category === 'All' ? ui.toolsLabel : `${getLocalizedCategoryLabel(category, locale)} ${ui.toolsLabel}`}
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 pb-20">
            {filteredTools.map((tool: ToolMeta) => (
              <Link key={tool.slug} href={localizedToolPath(locale, tool.slug)}
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-xs sm:text-sm font-bold truncate text-zinc-900 dark:text-white group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">
                    {getLocalizedToolName(tool, locale)}
                  </h2>
                  <span className="text-[10px] font-semibold text-violet-600 dark:text-violet-400 block mt-0.5">
                    {getLocalizedCategoryLabel(tool.category, locale)} {ui.toolsLabel}
                  </span>
                  <p className="mt-1 text-[11px] text-zinc-500 dark:text-zinc-400 line-clamp-1">
                    {getLocalizedToolSeoContent(tool, locale).intro}
                  </p>
                </div>
              </Link>
            ))}
