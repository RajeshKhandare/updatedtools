...
import { CalendarDays, Download, Printer, RefreshCw, Sparkles } from 'lucide-react';
...
export default function TimeTableEngine({tool,locale='en'}:{tool:ToolMeta;locale?:LocaleCode}){
  const ui=TT_UI[locale]||TT_UI.en;
  const preset=slugPreset(tool.slug);
  const [dayCount,setDayCount]=useState(preset.days);
  const [periodCount,setPeriodCount]=useState(preset.periods);
  const [start,setStart]=useState('08:00');
  const [duration,setDuration]=useState(50);
  const [breakAfter,setBreakAfter]=useState(4);
  const [subjects,setSubjects]=useState(preset.subjects.join(', '));
  ...
  const generate=()=>{
    const safeSubjects=subjectList.length?subjectList:['Study','Break','Personal'];
    const next=Array.from({length:dayCount},(_,d)=>Array.from({length:periodCount},(_,p)=>safeSubjects[(d*2+p)%safeSubjects.length]));
    setGrid(next);
    setGenerated(true);
  };

  const reset=()=>{
    setDayCount(preset.days); setPeriodCount(preset.periods); setStart('08:00'); setDuration(50); setBreakAfter(4);
    setSubjects(preset.subjects.join(', ')); setGrid([]); setGenerated(false);
  };

  const updateCell=(d:number,p:number,value:string)=>{
    setGrid(prev=>prev.map((row,ri)=>ri===d?row.map((cell,ci)=>ci===p?value:cell):row));
  };

  const download=()=>{
    if(!grid.length)return;
    const headers=[ui.weekday,...Array.from({length:periodCount},(_,p)=>`${ui.period} ${p+1}`)];
    const rows=grid.map((row,d)=>[DAY_LABELS[locale][d],...row]);
    const csv=[headers,...rows].map(row=>row.map(csvEscape).join(',')).join('\n');
    const blob=new Blob([csv],{type:'text/csv;charset=utf-8'});
    const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`${tool.slug}.csv`; a.click(); setTimeout(()=>URL.revokeObjectURL(url),500);
  };

  const times=Array.from({length:periodCount},(_,p)=>addMinutes(start,p*duration+(p>=breakAfter?20:0)));

  return <div className={card} data-testid="timetable-engine">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-violet-600 dark:text-violet-400"><CalendarDays className="h-4 w-4"/>{ui.title}</div><h2 className="mt-2 text-xl sm:text-2xl font-black">{getLocalizedToolName(tool,locale)}</h2><p className="mt-1 text-xs text-zinc-500">{ui.notes}</p></div>
      <div className="rounded-2xl bg-violet-50 dark:bg-violet-950/30 px-3 py-2 text-[11px] font-semibold text-violet-700 dark:text-violet-300"><Sparkles className="inline h-3.5 w-3.5 mr-1"/>{tool.slug==='smart-timetable-generator'?ui.smart:ui.ready}</div>
    </div>
    ...
    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <label className="text-xs font-semibold">{ui.periods}<select className={input+' mt-2'} value={periodCount} onChange={e=>setPeriodCount(Number(e.target.value))}>{[2,3,4,5,6,7,8,9,10].map(n=><option key={n} value={n}>{n}</option>)}</select></label>
      ...
      <label className="block text-xs font-semibold">{ui.subjects}<textarea className={input+' mt-2 min-h-24'} value={subjects} onChange={e=>setSubjects(e.target.value)} placeholder={ui.subjectsHint}/></label>
      <div className="flex flex-wrap gap-2">
        <button className={button} onClick={generate}><CalendarDays className="h-4 w-4"/>{ui.generate}</button>
    ...
    {generated&&grid.length>0?<div className="overflow-x-auto rounded-2xl border border-zinc-200 dark:border-zinc-800"><table className="w-full min-w-[760px] border-collapse text-xs"><thead><tr className="bg-zinc-100 dark:bg-zinc-800"><th className="border border-zinc-200 dark:border-zinc-700 p-3 text-left">{ui.weekday}</th>{Array.from({length:periodCount},(_,p)=><th key={p} className="border border-zinc-200 dark:border-zinc-700 p-3 text-left">{ui.period} {p+1}<span className="block text-[10px] font-normal text-zinc-400">{times[p]}</span></th>)}</tr></thead><tbody>{grid.map((row,d)=><tr key={d}><th className="border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 p-3 text-left font-bold">{DAY_LABELS[locale][d]}</th>{row.map((cell,p)=><td key={p} className="border border-zinc-200 dark:border-zinc-700 p-1"><input aria-label={`${DAY_LABELS[locale][d]} ${ui.period} ${p+1}`} className="w-full rounded-xl bg-transparent px-2 py-2 outline-none focus:bg-violet-50 dark:focus:bg-violet-950/30" value={cell} onChange={e=>updateCell(d,p,e.target.value)}/></td>)}</tr>)}</tbody></table></div>:<div className="rounded-2xl border border-dashed border-zinc-300 dark:border-zinc-700 p-8 text-center text-sm text-zinc-500">{ui.empty}</div>}

    <div className="rounded-2xl bg-zinc-50 dark:bg-zinc-950/70 p-4 text-xs text-zinc-500"><strong className="text-zinc-700 dark:text-zinc-300">{ui.custom}:</strong> {ui.subjectsHint}</div>
  </div>;
}
