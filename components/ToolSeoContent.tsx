...
function VisualPanel({ tool, visual }: { tool: ToolMeta; visual: ReturnType<typeof getToolSeoContent>['visual'] }) {
  ...
  if (visual === 'youtube') return <div className={common}><div className="flex items-center gap-2 mb-4"><Video className="h-5 w-5 text-violet-600" /><h3 className="font-bold">Creator workflow</h3></div><div className="grid grid-cols-3 gap-2 text-xs"><div className="rounded-2xl border p-4">Video/topic</div><div className="rounded-2xl border p-4 bg-violet-50 dark:bg-violet-950/30">Tool analysis</div><div className="rounded-2xl border p-4 bg-emerald-50 dark:bg-emerald-950/30">Creator output</div></div><p className="mt-3 text-[11px] leading-5 text-zinc-500">Public platform data and estimates can change; use outputs as planning aids.</p></div>;
