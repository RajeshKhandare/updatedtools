...
if(slugs.length!==112||new Set(slugs).size!==112) throw new Error(`Expected 112 unique tools; found ${slugs.length} / ${new Set(slugs).size}`);
...
if(names.length!==112||descriptions.length!==112||targetKeywords.length!==112) throw new Error(`Expected 112 names, descriptions, and target keywords; found ${names.length}, ${descriptions.length}, ${targetKeywords.length}`);
...
const expectedCounts={PDF:14,Image:14,Compiler:8,Developer:12,Text:10,Converters:10,Calculators:9,Finance:7,YouTube:3,'Time Table':25};
...
for(const c of ['PDF','Image','Compiler','Finance','Developer','Text','Converters','Calculators','YouTube','Time Table']) if(!runner.includes(`'${c}'`)) throw new Error(`Runner missing ${c}`);
